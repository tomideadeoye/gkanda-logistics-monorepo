#!/usr/bin/env python3
"""
Convert the ESIA HTML cover page to PDF using ReportLab.
"""
...
def create_esia_cover_pdf():
    ...
    try:
        ...
        pdf_file = os.path.join(base_dir, "ESIA_Cover.pdf")
        ...
        c.drawCentredString(page_width/2, page_height - margin_top - 90*mm, "ENVIRONMENTAL AND SOCIAL IMPACT ASSESSMENT")
        ...
        # Save the PDF
        c.save()
        print(f"Successfully created ESIA cover PDF: {pdf_file}")
        return True
        
    except Exception as e:
        print(f"Error creating ESIA cover PDF with ReportLab: {e}")
        return False

def main():
    """Main function to create the ESIA cover PDF."""
    print("Creating ESIA cover page PDF...")
    if create_esia_cover_pdf():
        print("ESIA cover page PDF created successfully!")
    else:
        print("Failed to create ESIA cover page PDF")
