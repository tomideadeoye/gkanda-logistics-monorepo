#!/usr/bin/env python3
"""
Convert the HR Policy Manual HTML cover page to PDF using ReportLab.
"""
...
def create_hr_cover_pdf():
    ...
    try:
        ...
        pdf_file = os.path.join(base_dir, "HR_Policy_Manual_Cover.pdf")
        ...
        c.drawCentredString(page_width/2, page_height - margin_top - 90*mm, "HUMAN RESOURCES POLICY MANUAL")
        ...
        # Save the PDF
        c.save()
        print(f"Successfully created HR Policy Manual cover PDF: {pdf_file}")
        return True
        
    except Exception as e:
        print(f"Error creating HR Policy Manual cover PDF with ReportLab: {e}")
        return False

def main():
    """Main function to create the HR Policy Manual cover PDF."""
    print("Creating HR Policy Manual cover page PDF...")
    if create_hr_cover_pdf():
        print("HR Policy Manual cover page PDF created successfully!")
    else:
        print("Failed to create HR Policy Manual cover page PDF")
