...
def convert_md_to_pdf_with_mermaid_cli(md_file_path, pdf_output_path):
    """Convert Markdown to PDF using Mermaid CLI directly"""
    print(f"Converting {os.path.basename(md_file_path)} to PDF with Mermaid CLI...")

    # Use mermaid CLI to convert directly to PDF
    cmd = [
        "npx",
...
def main():
    ...
    handbook_pdf = os.path.join(base_dir, "temp_handbook_mermaid_cli.pdf")
    ...
    if convert_md_to_pdf_with_mermaid_cli(md_file, handbook_pdf):
