...
def convert_md_to_pdf_with_nodejs(md_file_path, pdf_output_path):
    """Convert Markdown to PDF using Node.js and Puppeteer"""
    print(f"Converting {os.path.basename(md_file_path)} to PDF using Node.js...")
    ...
    try:
        # Run Node.js script
        cmd = ['node', script_path, md_file_path, pdf_output_path]
        ...
        if result.returncode == 0:
            ...
            print(f"Node.js conversion failed: {result.stderr}")
            return False

    except Exception as e:
        print(f"Error during Node.js conversion: {e}")
        return False
...
def main():
    ...
    handbook_pdf = os.path.join(base_dir, "temp_handbook_nodejs.pdf")

    # Final merged output
    final_pdf = os.path.join(base_dir, "GK_and_A_Logistics_ESMS_Handbook_NodeJS.pdf")

    # Step 1: Convert Markdown to PDF with Node.js
    print("Step 1: Converting Markdown to PDF with Node.js...")
    if convert_md_to_pdf_with_nodejs(md_file, handbook_pdf):
